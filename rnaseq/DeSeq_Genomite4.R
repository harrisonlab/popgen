...
write.table(countData,"mite_countData.txt",sep="\t",na="",quote=F)
...
orderedColData <- read.table("mite_deseq_samples.txt",header=T,sep="\t")
colData <- data.frame(orderedColData[ order(orderedColData$Sample.name),])
unorderedData <- read.table("mite_countData.txt",header=T,sep="\t")
...
pdf("mite_heatmap_vst.pdf", width=36,height=36)
...
rld <- rlog( dds )
pdf("mite_heatmap_rld.pdf", width=36,height=36)
...
##PCA plots
pdf("mite_PCA_vst.pdf", width=12,height=12)
plotPCA(vst,intgroup=c("Stress", "Mite", "Time"))
dev.off()

#Plot using rlog transformation:
pdf("mite_PCA_rld.pdf", width=12,height=12)
...
ggsave("mite_PCA_sample_names.pdf", pca_plot, dpi=300, height=12, width=12)
