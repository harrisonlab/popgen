...
$porechop/porechop-runner.py -i $RawReads -o "$Prefix"_trim.fastq.gz --threads 16 --adapter_threshold 80.0 > "$Prefix"_trim_log.txt

#######  Step 3	 ########
# Tidy up
#########################
cp "$Prefix"_trim.fastq.gz $CurDir
cp "$Prefix"_trim_log.txt $CurDir