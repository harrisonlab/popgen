...
GENOME.class.split <- F_ST.stats(GENOME.class.split, mode="nucleotide")
...
{
  file_hist <- paste(dir, "_", population_names[i], "_pairwise_FST_per_gene.pdf", sep="")
  ...
  ggsave(file_hist, fst_plot)
  file_table = paste(dir, "_", population_names[i], "_pairwise_FST_per_gene.txt", sep="")
  current_gff <- paste(gff, "/", dir, ".gff", sep="")
  gene_ids <- get_gff_info(GENOME.class.split, current_gff, chr=dir, feature=FALSE, extract.gene.names=TRUE)
...
{
  file_hist <- paste(dir, "_", population_names[i], "_pairwise_FST_per_sliding_window.pdf", sep="")
  ...
  ggsave(file_slide, slide_plot)
  #write table with raw data
  slide_table <- paste(dir, "_", population_names[i], "_pairwise_FST_per_sliding_window.txt", sep="")
